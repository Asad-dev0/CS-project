...
const normalizeBooking = (booking) => {
  ...
  // derive completed/upcoming from return date only for display, but preserve backend status
  try {
    const now = new Date();
    const _return = new Date(normalized.dates.return);
    if (normalized.status === "active" || normalized.status === "pending") {
      // Keep backend status for logic, but we can add a displayStatus if needed
      normalized.displayStatus = _return > now ? "upcoming" : "completed";
    }
  } catch {
    normalized.displayStatus = normalized.status || "upcoming";
  }
      ...
      <div className={s.cardContent}>
        ...
        <div className={s.cardActions}>
          ...
          <Link to="/cars" className={s.bookAgainButton}>
            <FaCar />
            {["pending", "active"].includes(booking.status) ? "goto cars" : "Book Again"}
          </Link>
        </div>
      </div>
    </div>
...
const BookingModal = ({ booking, onClose, onCancel, onModify }) => {
  const days = daysBetween(booking.dates.pickup, booking.dates.return);
          ...
          <div className={s.modalActions}>
            ...
            {["pending", "active"].includes(booking.status) && (
              <button
                ...
                onClick={() => {
                  onClose();
                  setTimeout(() => onModify(booking), 100);
                }}
    ...
    <div className={s.modalOverlay}>
      <div className={s.modalContainer}>
        <div className={s.modalContent}>
          <div className={s.modalHeader}>
            <h2 className={s.modalTitle}>
              <FaCalendarAlt className="text-orange-400" /> Modify Booking
            </h2>
            <button
              type="button"
              onClick={onClose}
          ...
          <div className={s.infoCard}>
            <div className="mb-4">
              <p className={s.carTitle}>{booking.car.make}</p>
              <p className={s.carSubtitle}>{booking.car.category}</p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  onChange={(e) => onModifyChange("pickupDate", e.target.value)}
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => onModifyChange("returnDate", e.target.value)}
                  className="w-full px-3 py-2 border border-gray-300 rounded focus:outline-none focus:border-orange-500"
                />
              </div>
            </div>
...
const MyBookings = () => {
  ...
  const cancelBooking = useCallback(
    ...
    () =>
      filter === "all"
        ? bookings
        : filter === "modifiable"
        ? bookings.filter((b) => ["pending", "active"].includes(b.status))
  ...
  const openModify = (b) => {
    ...
    setModifyData({
      pickupDate: b.dates.pickup
        ? new Date(b.dates.pickup).toISOString().split("T")[0]
        : "",
      returnDate: b.dates.return
        ? new Date(b.dates.return).toISOString().split("T")[0]
        : "",
    });
    setIsModifying(true);
    setShowModal(false);
  ...
  const closeModify = () => {
    ...
    setModifyData({ pickupDate: "", returnDate: "" });
  };

  const saveModify = useCallback(
    async (bookingId) => {
      if (!modifyData.pickupDate || !modifyData.returnDate) {
        alert("Please fill in all dates");
        return;
      }

      const pickupDate = new Date(modifyData.pickupDate);
      const returnDate = new Date(modifyData.returnDate);
  ...
  const filterButtons = [
    ...
    { key: "modifiable", label: "Modifiable", icon: <FaClock /> },
    ...
    <div className={s.pageContainer}>
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mt-12 grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          />
          <StatsCard
            value={bookings.filter((b) => ["pending", "active"].includes(b.status)).length}
            label="Modifiable Bookings"
            color="text-blue-400"
          />
        </div>
      </div>
