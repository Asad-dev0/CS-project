...
export const updateBooking = async (req, res, next) => {
  try {
    ...
    // Only allow modification before booking is completed or cancelled
    if (["completed", "cancelled"].includes(booking.status)) {
      return res.status(403).json({
        success: false,
        message: "Booking cannot be modified after completion or cancellation"
      });
    }

    // image handling logic here (if needed)
    // file handling
    if (req.file) {
